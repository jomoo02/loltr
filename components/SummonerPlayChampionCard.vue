<template>
    <div class="w-full xl:w-[22.5rem] h-14 xs:h-16 md:px-10 xl:px-0">
        <div class="w-full h-full flex justify-between px-3 xs:px-7">
            <div class="w-[6rem] xs:w-[8rem] flex items-center gap-x-1.5 xs:gap-x-2 ">
                <!-- 챔피언 아이콘 -->
                <img :src="getChampionIconUrl(playChampionName)" class="w-[1.85rem] h-[1.85rem] xs:w-[2.5rem] xs:h-[2.5rem]">
                <!-- 챔피언 이름 -->
            ...
            <div class="flex flex-col items-center justify-center">
                ...
                <span class="text-xxxs xs:text-xs font-medium text-gray-800">{{ playChampionInfo.cnt }} 게임</span>
            </div>
        </div>
    </div>
</template>
